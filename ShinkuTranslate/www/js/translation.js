(function() {
  ...
  renderParseResult = function(p) {
    ...
    breakIndex = -1;
    if (separateSpeaker) { // Check if separating speakers is enabled
    // Get the last part of the parse result
    lastPart = parseResult.parts[parseResult.parts.length - 1];
    // Check if the last part is not an array (indicating a single character)
    if (!_.isArray(lastPart)) {
        // Get the last character of the last part
        lastChar = lastPart.charAt(lastPart.length - 1);
        // Check if the last character is one of the specified closing punctuation marks
        if (lastChar === "』" || lastChar === "」" || lastChar === "）") {
            // Determine the corresponding opening punctuation mark based on the closing punctuation mark
            firstChar = (function() {
                switch (lastChar) {
                    case "』":
                        return "『";
            ...
            // Loop through the parse result parts to find the occurrence of the opening punctuation mark
            for (j = _i = 0, _ref = parseResult.parts.length; 0 <= _ref ? _i < _ref : _i > _ref; j = 0 <= _ref ? ++_i : --_i) {
                part = parseResult.parts[j];
                // Check if the current part is not an array (indicating a single character)
                if (!_.isArray(part)) {
                    // Find the index of the opening punctuation mark in the current part
                    firstCharIdx = part.indexOf(firstChar);
                    // Check if the opening punctuation mark is found and it's not at the beginning of the text
                    if (firstCharIdx !== -1 && !(j === 0 && firstCharIdx === 0)) {
                        // Replace the previous parts with blank spaces
                        var blankSpaces = '';
                        for (var k = 0; k < firstCharIdx; k++) {
                            blankSpaces += ' ';
                        }
                        parseResult.parts.splice(0, j, blankSpaces); // Replace previous parts with blank spaces
